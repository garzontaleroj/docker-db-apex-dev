...
EOF`
    ...
    if [ "$APEX_SCHEMA" = "APEX_050000" ]; then
        ...
        echo "Install AOP Sample App for APEX ${APEX_SCHEMA} (using latest available)"
        LATEST_APP=$(ls -d apex_version_*/ 2>/dev/null | sort -V | tail -1)
        if [ -n "${LATEST_APP}" ] && [ -f "${LATEST_APP}aop_sample_apex_app.sql" ]; then
            echo "@@${LATEST_APP}aop_sample_apex_app.sql" >> install_aop_app.sql
        else
            echo "-- No compatible AOP sample app found for ${APEX_SCHEMA}, skipping" >> install_aop_app.sql
        fi
    fi
