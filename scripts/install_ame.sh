...
EOF`
    ...
    if [ "$APEX_SCHEMA" = "APEX_180100" ]; then
        ...
        echo "Install AME Sample App for APEX ${APEX_SCHEMA} (using latest available)"
        if [ -f "ame_sample_apex_app.sql" ]; then
            echo "@@ame_sample_apex_app.sql" >> install_ame_app.sql
        else
            echo "-- No compatible AME sample app found for ${APEX_SCHEMA}, skipping" >> install_ame_app.sql
        fi
    fi
